    ...
    if (message.content) {
        for (const key in message.content) {
            ...
            if (length == 0) {
                ...
                const type = typeof message.content[key];
                if (key == '__kind') {
...
TmcCodec.prototype.readMessage = function(buffer) {
    ...
    try {
        let headerType = buffer.readUInt16LE(2);
        let index = 4;
        while (headerType != Common.enum.HeaderType.EndOfHeaders) {
            if (headerType === Common.enum.HeaderType.StatusCode) {
                ...
                var length = buffer.readUInt32LE(index);
                const key = buffer.toString('UTF-8', index + 4, index + length + 4);
                index = index + length + 4;

                const format = buffer.readUInt8(index);
                index += 1;
